...
class PlotConfig:
    def __init__(self, distribution: str, n: int, seed: int, lambda_val: int, R: int = 0, data_type: str = "uint64", algorithm: str = "greedy"):
        ...
        algorithm_cands = [
            ...
            "duplicate_allowed",
            "optimal_poison_duplicate_allowed",
        ...
        assert self.algorithm in algorithm_cands, f"Algorithm {self.algorithm} not found in {algorithm_cands}"
    ...
    def _get_poisoned_file_path(self) -> str:
        ...
        elif self.algorithm == "duplicate_allowed":
            if self.R == 0:
                return os.path.join(DATA_DIR, f"{self._distribution_to_str_on_file()}_n{self.n}_seed{self.seed}_lambda{self.lambda_val}_duplicate_allowed_{self.data_type}")
            else:
                return os.path.join(DATA_DIR, f"{self._distribution_to_str_on_file()}_n{self.n}_R{self.R}_seed{self.seed}_lambda{self.lambda_val}_duplicate_allowed_{self.data_type}")
        elif self.algorithm == "optimal_poison_duplicate_allowed":
            if self.R == 0:
                return os.path.join(DATA_DIR, f"{self._distribution_to_str_on_file()}_n{self.n}_seed{self.seed}_lambda{self.lambda_val}_optimal_poison_duplicate_allowed_{self.data_type}")
            else:
                return os.path.join(DATA_DIR, f"{self._distribution_to_str_on_file()}_n{self.n}_R{self.R}_seed{self.seed}_lambda{self.lambda_val}_optimal_poison_duplicate_allowed_{self.data_type}")
        elif self.algorithm == "consecutive_w_endpoints_duplicate_allowed":
            if self.R == 0:
                return os.path.join(DATA_DIR, f"{self._distribution_to_str_on_file()}_n{self.n}_seed{self.seed}_lambda{self.lambda_val}_consecutive_w_endpoints_duplicate_allowed_{self.data_type}")
            else:
                return os.path.join(DATA_DIR, f"{self._distribution_to_str_on_file()}_n{self.n}_R{self.R}_seed{self.seed}_lambda{self.lambda_val}_consecutive_w_endpoints_duplicate_allowed_{self.data_type}")
        else:
    ...
    def get_poisoned_data_with_label(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        leg_i = 0
        poi_i = 0
        labels = [0 for _ in range(len(poisoned_data))]
        while leg_i < len(legitimate_data) and poi_i < len(poisoned_data):
            if legitimate_data[leg_i] < poisoned_data[poi_i]:
                leg_i += 1
            elif legitimate_data[leg_i] > poisoned_data[poi_i]:
                poi_i += 1
            else:
                labels[poi_i] = 1
                leg_i += 1
                poi_i += 1
        while poi_i < len(poisoned_data):
            labels[poi_i] = 0
            poi_i += 1
...
def main():
    ...
    target_configs = [
        [
            ...
            PlotConfig('fb', 50, 11, 5, 0, "uint64", "duplicate_allowed"),
            PlotConfig('fb', 50, 11, 5, 0, "uint64", "optimal_poison_duplicate_allowed"),
            PlotConfig('fb', 50, 11, 5, 0, "uint64", "consecutive_w_endpoints_duplicate_allowed")
        ]
    ]
    row_titles = [""]
    column_seeds = [11, 11, 11, 11]
    plot_poisoned_datasets(target_configs, column_seeds, row_titles, "poisoned_datasets_rank_plots_duplicate_allowed_fb.pdf", xlim=None, ylim=None)
