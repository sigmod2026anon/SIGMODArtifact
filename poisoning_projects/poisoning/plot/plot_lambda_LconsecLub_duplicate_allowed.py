...
def plot_lambda_LconsecLub_duplicate_allowed(consecutive_w_endpoints_duplicate_allowed_df, df_upper_bound, fig_path, n=None, algorithm=None, show_boxplot=None):
    """
    Plot for each n. The x-axis of each graph is lambda. The y-axis displays MSE_{SEG+E}/MSE_{UB} for duplicate_allowed case.
    ...
    consecutive_w_endpoints_duplicate_allowed_df = consecutive_w_endpoints_duplicate_allowed_df.sort_values(by=["dataset_name", "n", "R", "seed", "data_type", "lambda"])
    df_upper_bound = df_upper_bound.sort_values(by=["dataset_name", "n", "R", "seed", "data_type", "lambda"])

    # Combine data for finding available combinations
    combined_df = pd.concat([consecutive_w_endpoints_duplicate_allowed_df, df_upper_bound], ignore_index=True)
    ...
    # Settings for LconsecE/Lub ratio plots (duplicate_allowed)
    ...
    for j, dist in enumerate(distributions):
        ...
        # Get data for consecutive_w_endpoints_duplicate_allowed and upper_bound approaches
        ...
        if consecutive_w_endpoints_duplicate_allowed_data.empty or upper_bound_data.empty:
        ...
        # Merge datasets
        merged_data = consecutive_w_endpoints_duplicate_allowed_data.merge(upper_bound_data, on=['lambda', 'seed'], how='inner')
        ...
        merged_data['LconsecE_duplicate_allowed_divided_by_Lub'] = merged_data['consecutive_w_endpoints_duplicate_allowed_loss'] / merged_data['upper_bound_loss']
        
        # Print minimum/maximum LconsecE_duplicate_allowed/Lub values for each lambda (percentage) and their corresponding seeds
        print(f"\n=== Minimum LconsecE_duplicate_allowed/Lub values for {dataset_name}, {data_type}, R={R} ===")
        ...
        y_min_0 = min(y_min_0, merged_data['LconsecE_duplicate_allowed_divided_by_Lub'].min())
        y_max_0 = max(y_max_0, merged_data['LconsecE_duplicate_allowed_divided_by_Lub'].max())

        # Plot LconsecE_duplicate_allowed/Lub ratio
        ax = axes[j]
        if show_boxplot:
            # Boxplot version for LconsecE_duplicate_allowed/Lub
            box_data_lub_duplicate_allowed = []
            box_positions = []
            for percentage in sorted(merged_data['percentage'].unique()):
                group_data = merged_data[merged_data['percentage'] == percentage]['LconsecE_duplicate_allowed_divided_by_Lub'].values
                if len(group_data) > 0:
                    box_data_lub_duplicate_allowed.append(group_data)
                    box_positions.append(percentage)
            
            if box_data_lub_duplicate_allowed:
                # Draw boxplot for LconsecE_duplicate_allowed/Lub
                bp = ax.boxplot(box_data_lub_duplicate_allowed, positions=box_positions, patch_artist=True, widths=0.8)
            ...
            # Mean/Min/Max version for LconsecE_duplicate_allowed/Lub
            stats_lub_duplicate_allowed = merged_data.groupby('percentage')['LconsecE_duplicate_allowed_divided_by_Lub'].agg(['mean', 'min', 'max'])
            
            ax.plot(stats_lub_duplicate_allowed.index, stats_lub_duplicate_allowed['mean'], 's-', label='Mean', color='blue', linewidth=2)
            ax.plot(stats_lub_duplicate_allowed.index, stats_lub_duplicate_allowed['min'], 'v:', label='Min', color='blue', linewidth=2)
            ax.plot(stats_lub_duplicate_allowed.index, stats_lub_duplicate_allowed['max'], 'v-.', label='Max', color='blue', linewidth=2)
        
        # Common settings for LconsecE_duplicate_allowed/Lub plot
        ...
        if j == 0:
            ax.set_ylabel(r'$\frac{\mathrm{MSE}_\mathrm{SEG{+}E(R.)}}{\mathrm{MSE}_\mathrm{UB}}$', fontsize=FONT_SIZE*1.5)
    ...
    # Print overall statistics for LconsecE_duplicate_allowed/Lub ratio
    if all_lconsecE_lub_ratios:
        ...
        print(f"Overall LconsecE_duplicate_allowed/Lub ratio statistics:")
...
if __name__ == '__main__':
    ...
    output_dir = "../results/fig/lambda_LconsecLub_duplicate_allowed"
    ...
    if consecutive_w_endpoints_duplicate_allowed_df.empty or df_upper_bound.empty:
    ...
    consecutive_w_endpoints_duplicate_allowed_df_ns = consecutive_w_endpoints_duplicate_allowed_df_ns.sort_values(by=["dataset_name", "n", "R", "seed", "data_type", "lambda"])
    df_upper_bound_ns = df_upper_bound_ns.sort_values(by=["dataset_name", "n", "R", "seed", "data_type", "lambda"])
    
    # Plot based on settings
    if BOXPLOT:
        plot_lambda_LconsecLub_duplicate_allowed(consecutive_w_endpoints_duplicate_allowed_df_ns, df_upper_bound_ns, 
                              f'{output_dir}/lambda_Lconsec_divided_by_Lub_duplicate_allowed_all.pdf', 
