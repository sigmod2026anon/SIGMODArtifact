...
# [POISON] Step 3.3: Inject poison into datasets using consecutive approach with endpoints (duplicate allowed)
...
echo "[POISON] Step 3.3: Injecting poison into datasets using consecutive approach with endpoints (duplicate allowed)..."
...
mkdir -p "$RESULTS_DIR/inject_poison_consecutive_w_endpoints_duplicate_allowed"
...
echo "[OK] Consecutive with endpoints poison injection (duplicate allowed) completed"
echo "  Results saved to: $RESULTS_DIR/inject_poison_consecutive_w_endpoints_duplicate_allowed/"

# Count JSON files
total_json_files=$(find "$RESULTS_DIR/inject_poison_consecutive_w_endpoints_duplicate_allowed/" -name "*.json" -type f | wc -l)
