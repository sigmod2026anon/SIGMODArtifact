...
int main(int argc, char* argv[]) {
    try {
        if (argc != 4) {
            ...
            std::cerr << "  Algorithm: Binary Search on Y-axis" << std::endl;
            return 1;
        }
        ...
        // Calculate upper bound (binary search based)
        auto [w_star, mse_star, time_taken] = calc_upper_bound_binary_search(xs, lambda, 1e-12, 50);
        ...
        //          << info.dataset_name << " (lambda=" << lambda << ", algorithm=binary_search)" << std::endl;
