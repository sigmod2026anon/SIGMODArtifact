...
# [POISON] Step 3: Inject poison into datasets
# This script injects poison into all generated datasets
...
echo "[POISON] Step 3: Injecting poison into datasets..."
...
# Function to inject poison into a dataset
run_inject_poison() {
    input_file_name="$1"
    output_file_name="$2"
    poison_num="$3"
    ...
    json_file_path="$RESULTS_DIR/inject_poison/${dataset_name}/n${n}/lambda${poison_num}/${json_file_name}"
    mkdir -p "$RESULTS_DIR/inject_poison/${dataset_name}/n${n}/lambda${poison_num}"
    ...
    rm -f "$json_file_path"
    ./inject_poison "$DATA_DIR/${input_file_name}" "$output_file_path" $poison_num "$json_file_path" 2>/dev/null || echo "      Error processing $dataset_name with lambda=$poison_num"
}
    ...
    for real_dataset_name in "${real_dataset_names[@]}"; do
        for seed in "${seeds[@]}"; do
            for dtype in "uint64"; do
                ...
                output_file_name="${real_dataset_name}_n${n_val}_seed${seed}_lambda${poison_num}_${dtype}"
                json_file_name="${real_dataset_name}_n${n_val}_seed${seed}_lambda${poison_num}_${dtype}.json"
                run_inject_poison "$input_file_name" "$output_file_name" "$poison_num" "$json_file_name" "$real_dataset_name" "$n_val"
            done
        done
    done
}
    ...
    for sync_dataset_name in "${sync_dataset_names[@]}"; do
        for seed in "${seeds[@]}"; do
            for dtype in "uint64"; do
                ...
                output_file_name="${sync_dataset_name}_n${n_val}_R${R_val}_seed${seed}_lambda${poison_num}_${dtype}"
                json_file_name="${sync_dataset_name}_n${n_val}_R${R_val}_seed${seed}_lambda${poison_num}_${dtype}.json"
                run_inject_poison "$input_file_name" "$output_file_name" "$poison_num" "$json_file_name" "$sync_dataset_name" "$n_val"
            done
        done
    done
}
...
echo "[OK] Poison injection completed"
echo "  Results saved to: $RESULTS_DIR/inject_poison/"

# Count JSON files
total_json_files=$(find "$RESULTS_DIR/inject_poison/" -name "*.json" -type f | wc -l)
