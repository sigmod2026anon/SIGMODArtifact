...
std::string get_data_output_filename(const std::string& filename, const size_t lambda) {
    ...
    if (pos != std::string::npos) {
        ...
        std::string new_basename = prefix + "_lambda" + std::to_string(lambda) + "_optimal_poison" + suffix;
        std::string dir_path = filename.substr(0, last_slash + 1);
        return dir_path + new_basename;
    }
    return "optimal_poison_" + basename;
}
...
int main(int argc, char* argv[]) {
    try {
        if (argc != 4) {
            ...
            std::cerr << "  Algorithm: Brute-force optimal poisoning" << std::endl;
            return 1;
        }
        ...
        if (dtype == common::DataType::UINT32) {
            ...
            std::vector<std::uint32_t> poisons = poisoning::get_optimal_poison_values_brute_force<std::uint32_t>(data, static_cast<size_t>(lambda), false);
            ...
            auto start = std::chrono::high_resolution_clock::now();
            std::vector<std::uint64_t> poisons = poisoning::get_optimal_poison_values_brute_force<std::uint64_t>(data, static_cast<size_t>(lambda), false);
            auto end = std::chrono::high_resolution_clock::now();
            time_taken_s = std::chrono::duration_cast<std::chrono::microseconds>(end - start).count() / 1000000.0;
