...
/**
 * minimax_quadratics_binary_search_over_y - Solve minimax problem for quadratic functions (binary search based)
 * 
 * min_w max_i (a_i * w^2 + b_i * w + c_i) 
 * Y-axis (t) is searched for the optimal solution
 * 
...
/**
 * calc_upper_bound_binary_search - Calculate upper bound (binary search based)
 * 
 * Use minimax algorithm based on binary search to calculate upper bound
 * 
...
template <typename T>
inline std::tuple<double, double, double> calc_upper_bound_binary_search(
    ...
    return calc_upper_bound_binary_search(xs_decimal, p, eps, max_iter);
}
