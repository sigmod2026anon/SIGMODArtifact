...
# [POISON] Step 3.4: Inject poison into datasets using consecutive approach with endpoints using relaxed solution
# This script injects poison into all generated datasets using consecutive poisoning algorithm with endpoints using relaxed solution
...
echo "[POISON] Step 3.4: Injecting poison into datasets using consecutive approach with endpoints using relaxed solution..."
...
mkdir -p "$RESULTS_DIR/inject_poison_consecutive_w_endpoints_using_relaxed_solution"

# Change to build directory
cd "$BUILD_DIR"

# Function to inject poison into a dataset using consecutive approach with endpoints using relaxed solution
run_inject_poison_consecutive_w_endpoints_using_relaxed_solution() {
    input_file_name="$1"
    output_file_name="$2"
    poison_num="$3"
    json_file_name="$4"
    ...
    json_file_path="$RESULTS_DIR/inject_poison_consecutive_w_endpoints_using_relaxed_solution/${dataset_name}/n${n}/lambda${poison_num}/${json_file_name}"
    mkdir -p "$RESULTS_DIR/inject_poison_consecutive_w_endpoints_using_relaxed_solution/${dataset_name}/n${n}/lambda${poison_num}"
    ...
    # command="./inject_poison_consecutive_w_endpoints_using_relaxed_solution \"$DATA_DIR/${input_file_name}\" \"$output_file_path\" $poison_num \"$json_file_path\""
    # echo "Command: $command"

    ./inject_poison_consecutive_w_endpoints_using_relaxed_solution "$DATA_DIR/${input_file_name}" "$output_file_path" $poison_num "$json_file_path" 2>/dev/null || echo "      Error processing $dataset_name with lambda=$poison_num"
}
    ...
    for real_dataset_name in "${real_dataset_names[@]}"; do
        for seed in "${seeds[@]}"; do
            for dtype in "uint64"; do
                ...
                output_file_name="${real_dataset_name}_n${n_val}_seed${seed}_lambda${poison_num}_consecutive_w_endpoints_using_relaxed_solution_${dtype}"
                json_file_name="${real_dataset_name}_n${n_val}_seed${seed}_lambda${poison_num}_consecutive_w_endpoints_using_relaxed_solution_${dtype}.json"
                run_inject_poison_consecutive_w_endpoints_using_relaxed_solution "$input_file_name" "$output_file_name" "$poison_num" "$json_file_name" "$real_dataset_name" "$n_val"
            done
        done
    done
}
    ...
    for sync_dataset_name in "${sync_dataset_names[@]}"; do
        for seed in "${seeds[@]}"; do
            for dtype in "uint64"; do
                ...
                output_file_name="${sync_dataset_name}_n${n_val}_R${R_val}_seed${seed}_lambda${poison_num}_consecutive_w_endpoints_using_relaxed_solution_${dtype}"
                json_file_name="${sync_dataset_name}_n${n_val}_R${R_val}_seed${seed}_lambda${poison_num}_consecutive_w_endpoints_using_relaxed_solution_${dtype}.json"
                run_inject_poison_consecutive_w_endpoints_using_relaxed_solution "$input_file_name" "$output_file_name" "$poison_num" "$json_file_name" "$sync_dataset_name" "$n_val"
            done
        done
    done
}
...
echo "[OK] Consecutive with endpoints using relaxed solution poison injection completed"
echo "  Results saved to: $RESULTS_DIR/inject_poison_consecutive_w_endpoints_using_relaxed_solution/"

# Count JSON files
total_json_files=$(find "$RESULTS_DIR/inject_poison_consecutive_w_endpoints_using_relaxed_solution/" -name "*.json" -type f | wc -l)
